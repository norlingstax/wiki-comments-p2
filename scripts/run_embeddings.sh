...
# options
CFG="configs/embeddings.yaml"
RUN_NAME=""

usage() {
  cat <<EOF
Usage: scripts/run_embeddings.sh [--cfg PATH] [--run-name STRING]
...
if [[ -n "$RUN_NAME" ]]; then
  "$PY" -m src.cli.run_embeddings --cfg "$CFG" --run-name "$RUN_NAME"
else
  "$PY" -m src.cli.run_embeddings --cfg "$CFG"
fi
