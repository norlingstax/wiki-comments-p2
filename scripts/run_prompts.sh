...
# options
CFG="configs/prompts.yaml"
RUN_NAME=""

usage() {
  cat <<EOF
Usage: scripts/run_prompts.sh [--cfg PATH] [--run-name STRING]
...
if [[ -n "$RUN_NAME" ]]; then
  "$PY" -m src.cli.run_prompts --cfg "$CFG" --run-name "$RUN_NAME" --prompt-type few_shot_4 --limit 500
else
  "$PY" -m src.cli.run_prompts --cfg "$CFG" --prompt-type few_shot_4 --limit 500
fi
